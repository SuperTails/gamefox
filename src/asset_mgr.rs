...
impl<'a> GraphicsHolder<'a> {
    ...
    pub fn load<T>(dir: &Path, creator: &'static TextureCreator<T>) -> Result<(), Mutex<GraphicsHolder<'static>>> {
        GRAPHICS_HOLDER.set(Mutex::new(GraphicsHolder::load_all(dir, creator)))
    }
...
static GRAPHICS_HOLDER: OnceCell<Mutex<GraphicsHolder<'static>>> = OnceCell::new();
